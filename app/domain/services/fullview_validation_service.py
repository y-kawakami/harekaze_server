...
USER_PROMPT: Final[str] = (
    ...
    "- 桜の木の幹（根元付近）から木の上部まで、木の現在の姿全体が概ね確認できる"
    "（樹冠が切断・欠損している場合は、現在の最上部まで確認できればよい）\n"
    "- 木の現在の姿（幹・主枝、および存在する場合は樹冠）が"
    "画像フレーム内に概ね収まっている\n"
    "\n"
    "## NG判定の条件\n"
    ...
    "3. **はみ出し**: 木の主要部分が画像フレームから大きくはみ出しており、"
    "全体像が確認できない"
    "（ただし、木の上部が物理的に切断・欠損している場合は"
    "はみ出しとは判定しないでください）\n"
    "\n"
    "## 判定の注意事項\n"
    ...
    "- **木の健康状態は判定対象外です**。枯れかけている木、樹冠が失われている木、"
    "上部が切断された木であっても、幹を含む木の現在の姿が写っていればOKとしてください\n"
    "\n"

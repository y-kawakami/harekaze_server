...
# bloom_status 更新スクリプトの実行
echo "bloom_status 更新スクリプトを実行しています..."
if ! .venv/bin/python scripts/update_bloom_status.py "$@"; then
    echo "bloom_status 更新に失敗しました"
    exit 1
fi
echo "bloom_status 更新が完了しました"
